...
public class AddCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        ...
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + COMPANY_DESC_TIKTOK + LINK_DESC_TIKTOK
                + EMAIL_DESC_TIKTOK + APPLIED_DATE_DESC_TIKTOK
        ...
        assertParseSuccess(parser, COMPANY_DESC_TIKTOK + LINK_DESC_TIKTOK + EMAIL_DESC_GOOGLE
                + EMAIL_DESC_TIKTOK + APPLIED_DATE_DESC_TIKTOK
                + TAG_DESC_FRIEND, new AddCommand(expectedInternship));

        // multiple addresses - last address accepted
        assertParseSuccess(parser, COMPANY_DESC_TIKTOK + LINK_DESC_TIKTOK + EMAIL_DESC_TIKTOK
                + APPLIED_DATE_DESC_GOOGLE + APPLIED_DATE_DESC_TIKTOK
                + TAG_DESC_FRIEND, new AddCommand(expectedInternship));
        ...
        assertParseSuccess(parser, COMPANY_DESC_TIKTOK + LINK_DESC_TIKTOK + EMAIL_DESC_TIKTOK
                + APPLIED_DATE_DESC_TIKTOK + TAG_DESC_HUSBAND
    ...
    public void parse_optionalFieldsMissing_success() {
        ...
        assertParseSuccess(parser, COMPANY_DESC_GOOGLE + LINK_DESC_GOOGLE
                        + EMAIL_DESC_GOOGLE + APPLIED_DATE_DESC_GOOGLE, new AddCommand(expectedInternship));
    }
    ...
    public void parse_compulsoryFieldMissing_failure() {
        ...
        assertParseFailure(parser, VALID_COMPANY_TIKTOK + LINK_DESC_TIKTOK
                        + EMAIL_DESC_TIKTOK + APPLIED_DATE_DESC_TIKTOK, expectedMessage);

        // all prefixes missing
        assertParseFailure(parser, VALID_COMPANY_TIKTOK + VALID_LINK_TIKTOK
                        + VALID_EMAIL_TIKTOK + VALID_APPLIED_DATE_TIKTOK, expectedMessage);
    }
    ...
    public void parse_invalidValue_failure() {
        ...
        // invalid link
        assertParseFailure(parser, COMPANY_DESC_TIKTOK + INVALID_LINK_DESC
                + EMAIL_DESC_TIKTOK + APPLIED_DATE_DESC_TIKTOK + TAG_DESC_HUSBAND
                + TAG_DESC_FRIEND, Link.MESSAGE_CONSTRAINTS);

        // invalid email
        assertParseFailure(parser, COMPANY_DESC_TIKTOK + LINK_DESC_TIKTOK
                + INVALID_EMAIL_DESC + APPLIED_DATE_DESC_TIKTOK + TAG_DESC_HUSBAND
                + TAG_DESC_FRIEND, Email.MESSAGE_CONSTRAINTS);

        // invalid applied date
        assertParseFailure(parser, COMPANY_DESC_TIKTOK + LINK_DESC_TIKTOK
                + EMAIL_DESC_TIKTOK + INVALID_APPLIED_DATE_DESC + TAG_DESC_HUSBAND
                + TAG_DESC_FRIEND, AppliedDate.MESSAGE_CONSTRAINTS);

        // invalid tag
        assertParseFailure(parser, COMPANY_DESC_TIKTOK + LINK_DESC_TIKTOK
                + EMAIL_DESC_TIKTOK + APPLIED_DATE_DESC_TIKTOK + INVALID_TAG_DESC
                + VALID_TAG_FRIEND, Tag.MESSAGE_CONSTRAINTS);

        // two invalid values, only first invalid value reported
        assertParseFailure(parser, COMPANY_DESC_TIKTOK + INVALID_LINK_DESC
                        + EMAIL_DESC_TIKTOK + INVALID_APPLIED_DATE_DESC, Link.MESSAGE_CONSTRAINTS);

        // non-empty preamble
        assertParseFailure(parser, PREAMBLE_NON_EMPTY + COMPANY_DESC_TIKTOK + LINK_DESC_TIKTOK
                + EMAIL_DESC_TIKTOK + APPLIED_DATE_DESC_TIKTOK + TAG_DESC_HUSBAND + TAG_DESC_FRIEND,
